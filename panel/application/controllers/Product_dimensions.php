...
class Product_dimensions extends MY_Controller
{
    ...
    public function delete($id)
    {
        ...
        if (!empty($product_dimension)) :
            ...
            if ($delete) :
                 ...
                 */
                $url = FCPATH . "uploads/{$this->viewFolder}/{$product_dimension->img_url}";
                if (!is_dir($url) && file_exists($url)) :
                    unlink($url);
                endif;
                $url = FCPATH . "uploads/{$this->viewFolder}/{$product_dimension->home_url}";
                if (!is_dir($url) && file_exists($url)) :
                    unlink($url);
                endif;
                $url = FCPATH . "uploads/{$this->viewFolder}/{$product_dimension->banner_url}";
                if (!is_dir($url) && file_exists($url)) :
                    unlink($url);
                endif;
                /**
                 * Remove Dimension From Product
                 */
                $this->products_w_dimensions_model->delete(["dimension_id" => $id]);
    ...
    public function getDimensions()
    {
        ...
        echo json_encode(["success" => true, "title" => "Başarılı!", "message" => "Ürün Ebatları Başarıyla Eşitlendi."]);
    }
}
