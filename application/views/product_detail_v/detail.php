                    ...
                    <div class="pcvContainer ms-2">
                        <?php if (!empty($sameVariants)) : ?>
                            ...
                            <?php foreach ($sameVariants as $sKey => $sValue) : ?>
                                <div class="pswItem">
                                    <input <?= clean($this->input->post("codes_id", true)) == $sValue->codes_id || $SV == 0 ? "checked" : NULL ?> type="radio" name="dimension" value="<?= $sValue->dimension_id ?>" id="<?= seo($sValue->dimension) ?>-<?= $sValue->dimension_id ?>">
                                    <label class="dimensionLabel" data-codes-id="<?= $sValue->codes_id ?>" for="<?= seo($sValue->dimension) ?>-<?= $sValue->dimension_id ?>"><?= $sValue->dimension ?></label>
                                </div>
                                <?php $SV++ ?>
                            <?php endforeach ?>
                        <?php endif ?>
                ...
                <?php if ($product->dimension_type == "ROLL") : ?>
                    ...
                    <?php if (!empty($cuts)) : ?>
                        <div class="pcVariation pcv2">
                            <span><?= lang("cutType") ?> : </span>
                            <?php $CV = 0 ?>
                            <div class="ms-2">
                                <?php foreach ($cuts as $cKey => $cValue) : ?>
                                    <div class="my-2">
                                        <input <?= $CV == 0 ? "checked" : null ?> required type="radio" name="cutType" value="<?= $cValue->title ?>" id="<?= seo($cValue->title) ?>-<?= $cValue->id ?>">
                                        <label class="ms-2" for="<?= seo($cValue->title) ?>-<?= $cValue->id ?>"><?= $cValue->title ?></label>
...
<?php endif ?>
<?= $this->load->view("includes/productSlider", ["title" => lang("sameProducts"), "data" => $same_products], true); ?>
...
<script>
    window.addEventListener('DOMContentLoaded', () => {
        ...
        if ($("input[name='height']").length) {
            $(document).on("keyup change", "input[name='height']", function() {
        ...
        $(document).on("click", ".addToCart", function(e) {
            ...
            let orderNote = '';
            <?php if ($product->dimension_type == "ROLL") : ?>
                orderNote += "<?= lang("cutType") ?>: " + $("input[name='cutType']:checked").val() + " / ";
            <?php endif ?>
            orderNote += ("<?= lang("orderNote") ?>: " + $("textarea[name='orderNote']").val());
