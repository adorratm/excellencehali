...
<?php foreach ($this->cart->contents() as $items) : ?>
    ...
    <?php if (!empty($product)) : ?>
        ...
        <div class="cartWidgetProduct">
            <a rel="dofollow" href="<?= base_url(lang("routes_product_collections") . "/" . lang("routes_product") . "/{$product->url}") ?>" title="<?= stripslashes($items["name"]) ?>"><img width="1920" height="1280" loading="lazy" data-src="<?= get_picture("products_v", $product->img_url) ?>" alt="<?= $items['name']; ?>" class="img-fluid lazyload"></a>
